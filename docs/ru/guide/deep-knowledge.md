...
Запуск юнита внутри `scoped(scope, () => unit(payload))` — самая явная граница, потому что scope передается напрямую. Он запускает работу графа в заданном scope, а возвращённый промис ждет завершения асинхронной работы, поднятой этим запуском.
