...
When `ChatPanel` unmounts, the cached model stays alive. When the same key appears again in the same scope, Vue gets the existing model instead of creating a fresh one.

You can also cache with `useModel` by passing `{ cache, key }`:
